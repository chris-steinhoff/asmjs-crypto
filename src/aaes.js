...
var Aes = function() {
	...
	function aesAsm(stdlib, foreign, heap) {
		...
		var Xtime3Sbox = Xtime2Sbox + 256;
		var Xtime2 = Xtime3Sbox + 256;
		var Xtime9 = Xtime2 + 256;
		var XtimeB = Xtime9 + 256;
...
view8[2310]=0x20; view8[2311]=0x40; view8[2312]=0x80; view8[2313]=0x1B; view8[2314]=0x36;
		...
		function expandKey(rk, key) {
			...
			for(idx = nk; (idx|0) < (nb * (nr + 1)) ; idx = (idx + 1)|0) {
				...
				tmp1 = view8[rk + (4 * idx - 3)];
				tmp2 = view8[rk + (4 * idx - 2)];
				tmp3 = view8[rk + (4 * idx - 1)];
				if((idx % nk) == 0) {
					tmp4 = tmp3;
					tmp3 = view8[Sbox + tmp0];
					tmp0 = view8[Sbox + tmp1] ^ view8[rcon + (idx / nk)];
					tmp1 = view8[Sbox + tmp2];
					tmp2 = view8[Sbox + tmp4];
				} else if(nk > 6 && idx % nk == 4) {
					tmp0 = view8[Sbox + tmp0];
					tmp1 = view8[Sbox + tmp1];
					tmp2 = view8[Sbox + tmp2];
					tmp3 = view8[Sbox + tmp3];
				}

				view8[rk + (4 * idx + 0)] = view8[rk + (4 * idx - 4 * nk + 0)] ^ tmp0;
				view8[rk + (4 * idx + 1)] = view8[rk + (4 * idx - 4 * nk + 1)] ^ tmp1;
				view8[rk + (4 * idx + 2)] = view8[rk + (4 * idx - 4 * nk + 2)] ^ tmp2;
				view8[rk + (4 * idx + 3)] = view8[rk + (4 * idx - 4 * nk + 3)] ^ tmp3;
			}
		}
